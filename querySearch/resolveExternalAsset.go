...
// ResolveExternalAsset resolves external assets and their sub-assets based on the given configuration.
// It iterates through the `config.Resolve` slice and resolves assets recursively.
func (q *QuerySearch) ResolveExternalAsset(stub *sw.StubWrapper, m *map[string]interface{}) (map[string]interface{}, errors.ICCError) {
	data := *m

	// Remove tags from the data before resolving assets.
	q.removeTags(&data)

	// Iterate through the `config.Resolve` slice to resolve assets.
	for _, resolveStr := range q.config.Resolve {
		...
		if len(tuplaKeys) > 1 {
			...
			// Create a list of sub-assets to resolve.
			...
			case map[string]interface{}:
				{
					...
					// Remove tags from the resolved asset.
...
// resolveSlice resolves assets within a slice.
// This function can handle two types of slices:
// 1. Reference slice for an asset
// 2. Slice containing a map with @key and other data at the same level
...
// resolveAsset resolves an asset and its sub-assets based on the given configuration.
